...
# ── 5. auto-generate dashboard credentials ──────────────────
DASH_KEY="$(node -e 'process.stdout.write(require("crypto").randomBytes(8).toString("hex"))')"
DASH_PASS="$(node -e 'process.stdout.write(require("crypto").randomBytes(5).toString("hex"))')"

# ── 6. write .env (locked down) ─────────────────────────────
umask 077
cat > .env <<ENV
...
UNIT
  ...
  nohup node bot.js >/tmp/owntown_v23.log 2>&1 &
...
${c_grn}════════════════════════════════════════════════════════${c_rst}
 ${c_grn}RY GROUP — Owntown Bot is live!${c_rst}
  • Dashboard : http://<server-ip>:8899   (login: admin / $DASH_PASS)
  • Telegram  : open your bot and send  /start   (learns your chat id)
  • Commands  : /status /selfix /errors /balance /help
